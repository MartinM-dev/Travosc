...
extern "C" {
	...
	void tosc_getValueMessage(tosc_message* osc, int32_t* intvalue);
...
void tosc_getValueMessage(tosc_message* osc, int32_t* valueOsc)
{
	...
	for (int i = 0; osc->format[i] != '\0'; i++) {
		switch (osc->format[i]) {
		case 'f':
			*valueOsc = tosc_getNextFloat(osc);
			break;
		case 'i':
			*valueOsc = tosc_getNextInt32(osc);
			break;
		}
	}
	printf("\n");
